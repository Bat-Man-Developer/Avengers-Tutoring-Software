...
<body>
    <div class="business-rain" id="business-rain"></div>
    <div class="container">
        <div class="business-icon">💼</div>
        <h1 class="title">The World Of Business</h1>
        <p class="subtitle">Preparing your lesson...</p>
        <div class="loading-bar"></div>
        <div id="concept-entries"></div>
    </div>

    <script>
        // Redirect after 5 seconds
        setTimeout(function() {
            window.location.href = 'businessstudiestest.php';
        }, 5000);

        // Business terms rain effect
        const businessRain = document.getElementById('business-rain');
        const terms = "💼📊📈💰£$€¥";
        ...
        function createRain() {
            ...
            let content = '';
            for (let i = 0; i < 20; i++) {
                content += terms[Math.floor(Math.random() * terms.length)] + '<br>';
            }
            column.innerHTML = content;

            businessRain.appendChild(column);
            setTimeout(() => businessRain.removeChild(column), 10000);
        }
        ...
        const conceptsContainer = document.getElementById('concept-entries');
        let currentConcept = 0;

        function showNextConcept() {
            const div = document.createElement('div');
            div.className = 'concept-entry';
            div.textContent = concepts[currentConcept];
            conceptsContainer.appendChild(div);

            if (conceptsContainer.children.length > 3) {
                conceptsContainer.removeChild(conceptsContainer.children[0]);
            }

            currentConcept = (currentConcept + 1) % concepts.length;
        }

        setInterval(showNextConcept, 2000);
    </script>
</body>
</html>